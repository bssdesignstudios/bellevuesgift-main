...
export default function TrackOrderPage() {
  ...
  const [searchedOrder, setSearchedOrder] = useState<string | null>(searchParams.get('order'));

  const { data: order, isLoading } = useQuery({
    queryKey: ['track-order-public', searchedOrder],
    queryFn: async () => {
    ...
    <div className="container mx-auto px-4 py-8 max-w-3xl animate-fade-in">
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground">
          Enter your order number or pickup code to check the status
        </p>
      </div>
                ...
                {steps.map((step, index) => {
                  ...
                  return (
                    <div key={step.key} className="flex flex-col items-center flex-1">
                      ...
                      <span className={`text-sm text-center ${isCurrent ? 'font-semibold' : 'text-muted-foreground'}`}>
                        {step.label}
                      </span>
                    </div>
                  );
                ...
                <div className="flex flex-col items-center gap-6">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-sm text-muted-foreground mt-4 max-w-sm">
                      Show this QR code or tell the cashier your pickup code when you arrive
                    </p>
                  </div>
