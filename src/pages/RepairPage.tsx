...
export default function RepairPage() {
  ...
  const { data: searchedTicket, isLoading: isSearching, refetch: searchTicketRefetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from('repair_tickets')
        .select('*')
        .eq('ticket_number', searchTicket.toUpperCase().trim())
        .maybeSingle();
  ...
  const submitMutation = useMutation({
    mutationFn: async (data: RepairFormData) => {
      ...
      const { data: ticketNumber } = await supabase.rpc('generate_repair_ticket_number');
    ...
    onError: (error: any) => {
      toast.error('Failed to submit repair request: ' + error.message);
    },
  });
