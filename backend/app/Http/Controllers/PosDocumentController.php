...
class PosDocumentController extends Controller
{
    ...
    public function searchQuotes(Request $request)
    {
        ...
        return response()->json(
            $query->limit(20)->get()->map(fn($quote) => [
                ...
                'status'       => $quote->status,
                'customer'     => $quote->customer ? ['id' => $quote->customer->id, 'name' => $quote->customer->name] : ($quote->customer_name ? ['id' => $quote->customer_id, 'name' => $quote->customer_name] : null),
                'total'        => $quote->total,
    ...
    public function searchInvoices(Request $request)
    {
        ...
        return response()->json(
            $query->limit(20)->get()->map(fn($inv) => [
                ...
                'status'         => $inv->status,
                'customer'       => $inv->customer ? ['id' => $inv->customer->id, 'name' => $inv->customer->name] : ($inv->customer_name ? ['id' => $inv->customer_id, 'name' => $inv->customer_name] : null),
                'total'          => $inv->total,
    ...
    public function getInvoiceItems(Request $request, $id)
    {
        ...
        return response()->json([
            ...
            'customer'       => $invoice->customer ? ['id' => $invoice->customer->id, 'name' => $invoice->customer->name] : ($invoice->customer_name ? ['id' => $invoice->customer_id, 'name' => $invoice->customer_name] : null),
            'total'          => $invoice->total,
